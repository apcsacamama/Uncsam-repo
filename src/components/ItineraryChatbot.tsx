...
export default function ItineraryChatbot({ selectedDestinations, customItinerary, isVisible, onClose, travelDate }: any) {
  ...
  const generateItinerary = async (isRevision: boolean = false, customInstruction: string = "") => {
    ...
    try {
        ...
        if (response.type === 'inquiry') {
            ...
            // 1. Just answer the question. DO NOT COUNT REVISION.
  ...
  // ... (Keep Manual Edits, Undo, GeneratePDF, and UI exactly as before) ...
  ...
  const handleManualEditSave = (dayIndex: number) => {
      ...
      if (day && day.items) {
          ...
          if (itemIndex > -1) { day.items[itemIndex].activity = editValue; setCurrentItinerary(newItinerary); setChatMessages(prev => [...prev, { role: 'ai', text: "Saved.", timestamp: new Date() }]); }
                  ...
                  <div key={idx} className="space-y-4">
                      ...
                      <Card className="border-none shadow-sm"><CardContent className="p-0"><div className="p-4">
                          ...
                          {day.items?.filter((i:any) => i.activity && i.activity.length > 2).map((item:any, i:number) => (
                              ...
                              <div key={i} className="flex gap-4 py-3 border-b last:border-0 group relative">
                                  ...
                                  <div className="flex-1">
                                      ...
                                      {editingItemId === item.id ? (
                                          ...
                                          <div className="flex gap-2"><Input value={editValue} onChange={(e) => setEditValue(e.target.value)} className="h-8"/> <Button size="sm" onClick={() => handleManualEditSave(idx)}>Save</Button></div>
                                  ...
                                  <div className="absolute right-0 top-3 opacity-0 group-hover:opacity-100 flex gap-1">
                                      ...
                                      <Button variant="ghost" size="icon" onClick={() => { setEditingItemId(item.id); setEditValue(item.activity); }}><Pencil className="w-3 h-3 text-blue-600"/></Button>

                                      <Button variant="ghost" size="icon" onClick={() => handleManualDelete(idx, item.id)}><Trash2 className="w-3 h-3 text-red-600"/></Button>
