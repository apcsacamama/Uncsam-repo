  ...
  useEffect(() => {
    ...
    const loadFavorites = () => {
      const saved = JSON.parse(
        localStorage.getItem("tour_favorites") || "[]"
      );
      setFavorites(saved);
    };
        ...
        <Tabs defaultValue="bookings" className="space-y-6">
            ...
            {liveBookings.length > 0 ? (
                  ...
                  <CardContent className="p-6">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-6 mb-6">
                      ...
                      <div className="flex items-center gap-3 text-sm">
                        ...
                        <div>
                          <p className="text-gray-500 font-medium">Group Size</p>
                          <p className="font-bold text-gray-900">
                            {booking.pax_count} Travelers
                          </p>
                        </div>
                      </div>
                    </div>
                        ...
                        <DialogContent className="max-w-xl">
                          <DialogHeader>
                            <DialogTitle className="flex items-center gap-2">
                              <Info className="w-5 h-5 text-blue-600" /> Full
                              Booking Overview
                            </DialogTitle>
                            <DialogDescription>
                              Detailed itinerary and destination breakdown.
                            </DialogDescription>
                          </DialogHeader>
                          <div className="space-y-6 py-4">
                            <div className="bg-gray-50 p-5 rounded-xl space-y-4 border">
                              <h4 className="font-bold text-gray-900 flex items-center gap-2">
                                <MapPin className="w-4 h-4 text-red-600" /> Planned
                                Itinerary
                              </h4>
                              <ul className="space-y-2">
                                {getDestinationNames(booking.destinations).map(
                                  (name, i) => (
                                    <li
                                      key={i}
                                      className="text-sm flex items-center gap-3 bg-white p-3 rounded-lg border shadow-sm"
                                    >
                            ...
                            <div className="bg-blue-50 p-4 rounded-lg text-xs text-blue-800 border border-blue-100 flex gap-3">
                              ...
                              <p className="leading-relaxed font-medium">
                                Your driver will reach out to you via email or
                                phone 24 hours before departure to coordinate the
              ...
              <div className="text-center py-24 bg-white rounded-xl border-2 border-dashed border-gray-200">
                ...
                <p className="text-gray-500 mb-6">
                  Explore our tour packages and start your Japan adventure.
                </p>
                <Button
          ...
          <TabsContent value="favorites" className="space-y-6">
            <div className="grid md:grid-cols-2 gap-6">
              <Card>
                <CardHeader>
                  <CardTitle className="flex items-center gap-2">
                    <Heart className="w-5 h-5 text-red-600" /> Saved Destinations
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="space-y-3">
                    {favoriteDestinations.map((dest, i) => (
                      <div
                        key={i}
                        className="flex justify-between items-center p-4 border rounded-xl bg-white hover:shadow-sm transition-shadow"
                      >
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="flex items-center gap-2">
                    <Star className="w-5 h-5 text-yellow-500" /> Saved Packages
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="space-y-3">
          ...
          <TabsContent value="settings">
            <Card className="max-w-md shadow-sm">
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                  <Settings className="w-5 h-5 text-gray-600" /> Account Security
                </CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
                ...
                <Button
                  variant="outline"
                  className="w-full justify-start font-bold text-red-600 border-red-600 hover:bg-red-50"
                  onClick={handleSignOut}
                >
                ...
                <Button
                  variant="ghost"
                  className="w-full justify-start text-xs text-gray-400 hover:text-red-500"
                >
