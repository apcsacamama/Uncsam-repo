...
import { createClient } from "jsr:@supabase/supabase-js@2"; // INJECTED: Supabase Client
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // =========================================================================
    // NEW: ISOLATED KNOWLEDGE INJECTOR (RAG Phase 2)
    // =========================================================================
    if (action === "add-document") {
      try {
        ...
        // 1. Ask Gemini to turn the text into an AI Vector using the NEW 2026 MODEL
        const embedRes = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-embedding-001:embedContent?key=${GEMINI_API_KEY}`, {
          ...
          body: JSON.stringify({
            ...
            outputDimensionality: 768 // Required to fit our database schema
          }),
        });

        const embedData = await embedRes.json();

        // DEBUG: If Google rejects it, throw the exact Google error message
        if (embedData.error) {
          return new Response(JSON.stringify({ error: `Google API Error: ${embedData.error.message}` }), { status: 500, headers: corsHeaders });
        }
        ...
        // Save the text AND the math into your Supabase database
